...
public class PdsFile {
	...
	public static void save(
		...
		// 파일들을 저장하고 Files Table 에 저장할 정보근 map 에 담는다.
		List<FilesDto> fileList = new ArrayList<>();
		
		// uploadfiles 에 넘어온 파일별로 반복 - 파일칸 n개를 다 못채워도 보내는 방법
		for (MultipartFile uploadfile : uploadfiles) {
			...
			// 저장된 파일의 정보를 FilesDto 에 파일정보로 저장
	...
	private static String makeFolder(String uploadPath) {
		...
		if( !uploadPathFolder.exists() )
			uploadPathFolder.mkdirs(); // 폴더 생성 make directory
		// mkdir()  : 상위 폴더가 없으면 폴더 생성 실패
		// mkdirs() : 상위 폴더가 없으면 폴더 전체를 생성한다.
